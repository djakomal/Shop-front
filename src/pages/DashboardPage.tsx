...
const TipRevenue = ({ active, payload, label }: TProps) => {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #bbf7d0', borderRadius: 10, padding: '8px 14px', boxShadow: '0 4px 16px rgba(22,163,74,0.15)' }}>
      ...
      <p style={{ fontWeight: 700, color: '#0f0f0f', fontSize: 14 }}>{fmt(payload[0].value)}</p>
    </div>
  );
};
const TipCount = ({ active, payload, label }: TProps) => {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #bbf7d0', borderRadius: 10, padding: '8px 14px', boxShadow: '0 4px 16px rgba(22,163,74,0.15)' }}>
      <p style={{ color: '#16a34a', fontSize: 11, marginBottom: 3, fontWeight: 500 }}>{label}</p>
      <p style={{ fontWeight: 700, color: '#0f0f0f', fontSize: 14 }}>{payload[0].value} vente(s)</p>
    </div>
  );
};
...
function StatCard({ icon, label, value, sub, bg, iconColor, border }: {
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 22, fontWeight: 700, color: '#0f0f0f', lineHeight: 1.2 }}>{value}</p>
      {sub && <p style={{ fontSize: 11, color: '#737373', marginTop: 4 }}>{sub}</p>}
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 256 }}>
      <div style={{ width: 44, height: 44, border: '3px solid #f5f5f5', borderTopColor: '#16a34a', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
    </div>
  );
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0f0f0f', marginBottom: 4 }}>Bonjour, {user?.name?.split(' ')[0]} 👋</h1>
            <p style={{ fontSize: 13, color: '#16a34a' }}>{new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 12, background: '#f5f5f5', border: '1px solid #bbf7d0' }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#737373' }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: '#1a1a1a' }}>En service</span>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6" style={{ background: '#fff', border: '1px solid #f5f5f5', borderRadius: 14 }}>
          <div style={{ marginBottom: 20 }}>
            <h2 style={{ fontSize: 15, fontWeight: 600, color: '#0f0f0f', display: 'flex', alignItems: 'center', gap: 6 }}>
  ...
  if (isAdmin && stats) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0f0f0f', marginBottom: 4 }}>Tableau de bord</h1>
            <p style={{ fontSize: 13, color: '#16a34a' }}>{new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 12, background: '#f5f5f5', border: '1px solid #bbf7d0' }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#737373' }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: '#1a1a1a' }}>Système actif</span>
          </div>
        </div>

        {/* 3 KPI — exactement comme le screenshot */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatCard icon={<IcoTrend/>}                                             label="Revenus totaux"    value={fmt(stats.total_revenue)}      bg="#f5f5f5" iconColor="#16a34a" border="#bbf7d0" sub={`${stats.total_sales} ventes`} />
          <StatCard icon={<IcoUsers/>}                                             label="Agents actifs"     value={String(stats.agents_count)}    bg="#f5f5f5" iconColor="#16a34a" border="#bbf7d0" sub="vendeurs" />
          <StatCard icon={<IcoAlert color={stats.low_stock_count > 0 ? '#ca8a04' : '#16a34a'} />}
                    ...
                    bg={stats.low_stock_count > 0 ? '#fef9c3' : '#f5f5f5'}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4">
          ...
          {/* BarChart revenus */}
          <div className="p-4 sm:p-6 min-w-0" style={{ background: '#fff', border: '1px solid #f5f5f5', borderRadius: 14 }}>
          ...
          {/* Top produits */}
          <div className="p-4 sm:p-6 min-w-0" style={{ background: '#fff', border: '1px solid #f5f5f5', borderRadius: 14 }}>
            <h2 style={{ fontSize: 15, fontWeight: 600, color: '#0f0f0f', marginBottom: 4 }}>Top produits</h2>
            ...
            {stats.top_products?.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {stats.top_products.map((p, i) => {
                  ...
                  const shades = ['#0f0f0f', '#1a1a1a', '#404040', '#737373', '#a3a3a3'];
                  return (
                    <div key={i}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, fontSize: 12 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ width: 22, height: 22, borderRadius: '50%', background: '#f5f5f5', color: '#166634', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</span>
                          <span style={{ fontWeight: 500, color: '#0f0f0f', maxWidth: 110, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</span>
                        </div>
            ...
            <div style={{ width: '100%', height: 150 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={days} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="sales" stroke="#737373" strokeWidth={2.5} dot={{ fill: '#16a34a', r: 4, strokeWidth: 0 }} activeDot={{ r: 6, fill: '#1a1a1a', strokeWidth: 0 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 16 }}>
          {[
            ...
            <div key={item.label} style={{ background: '#fff', border: '1px solid #f5f5f5', borderRadius: 14, padding: 18 }}>
              <p style={{ fontSize: 12, color: '#16a34a', marginBottom: 10 }}>{item.label}</p>
              <p style={{ fontSize: 18, fontWeight: 700, color: '#0f0f0f', lineHeight: 1.2 }}>{item.value}</p>
              <p style={{ fontSize: 11, color: '#737373', marginTop: 4 }}>{item.sub}</p>
            </div>
          ))}
        </div>
